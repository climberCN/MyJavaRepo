...
public class UseGenericClassAppMain {
	...
	private static void useStringList() {
		...
		for (int i = 0; i < strList.size(); i++) {
			// 必须强制类型转换，心好累，我明明知道里面只有String的呀
	...
	// 我知道没用，但是我得告诉Java说这个List里只有String，方法就是使用泛型
	private static List<String> createStringListGenerics() {
		// 泛型的语法就是在支持泛型的类型上，给出类型的定义
		// List接口是支持泛型的，类型就是List里允许的元素类型
		// 创建List实例和引用的时候，都可以（非必要）指定泛型对应的类型
		// 不指定，那就是Object，所以我们之前用的List的元素就是Object类型
		List<String> ret = new ArrayList<String>();
		for (int i = 0; i < 10; i++) {
			// 如果尝试增加非String类型的元素进去，则会报错
			ret.add("str" + (i % 5));
//			ret.add(new Object());
		}
