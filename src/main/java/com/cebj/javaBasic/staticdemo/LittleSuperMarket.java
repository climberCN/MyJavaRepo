...
public class LittleSuperMarket {
    ...
    public LittleSuperMarket(String superMarketName, String address, int parkingCount, int merchandisesCount, int count) {
        this.superMarketName = superMarketName;
        this.address = address;
        this.parkingCount = parkingCount;

        merchandises = new MerchandiseV2WithStaticVariable[merchandisesCount];
        for (int i = 0; i < merchandises.length; i++) {
            double purchasePrice = Math.random() * 200;
            MerchandiseV2WithStaticVariable m = new MerchandiseV2WithStaticVariable("商品" + i,
                    "ID" + i,
                    count,
