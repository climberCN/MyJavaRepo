...
public class MerchandiseV2WithConstructor {
    ...
    // >> TODO 构造方法可以有参数，规则和语法与普通方法一样，而且构造方法没有返回值。使用时，参数传递给new语句后类名的括号中。
    // >> TODO 如果类中没有显式地添加一个构造方法，Java会给每个类都默认自带一个无参的构造方法。
    // TODO 如果我们自己添加类的构造方法，Java就不会再添加无参的构造方法。这时候，就不能直接new一个对象而不传递参数了
    // >> TODO 所以我们一直都在使用构造方法，这也是为什么创建对象的时候，类名后面要有一个括号的原因。
    // >> TODO 构造方法无法被点操作符调用，或者在普通方法里调用，只能通过new语句在创建对象的时候，间接调用。
