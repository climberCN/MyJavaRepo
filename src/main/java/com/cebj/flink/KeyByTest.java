...
public class KeyByTest {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<SensorRecord> sensorStream = dataStream.map(new TestMapFunction());
        // 之后我们想让不同的数据进入不同的分区
//        KeyedStream<SensorRecord, String> sensorRecordStringKeyedStream = sensorStream.keyBy(new MySelector());
        KeyedStream<SensorRecord, Tuple> sensorRecordStringKeyedStream = sensorStream.keyBy("id");
//        SingleOutputStreamOperator<SensorRecord> temperatureMax = sensorRecordStringKeyedStream.maxBy("temperature");
        SingleOutputStreamOperator<SensorRecord> reduceStream = sensorRecordStringKeyedStream.reduce(new MyReduceFunction());
        reduceStream.print();
        env.execute();
    }
    ...
    private static class MyReduceFunction implements ReduceFunction<SensorRecord> {
        ...
        public SensorRecord reduce(SensorRecord value1, SensorRecord value2) throws Exception {
            Long maxTimestamp = value1.getTimestamp().compareTo(value2.getTimestamp()) > 0 ? value1.getTimestamp() : value2.getTimestamp();
            Double minTemperature = value1.getTemperature().compareTo(value2.getTemperature()) < 0 ? value1.getTemperature() : value2.getTemperature();
